...
public class GamePanel extends JPanel implements ActionListener{
    ...
    boolean running,can_hit=false,can_start=true,can_stand=true;
    static final String path="C://Users//srava//Documents//Java Project//BJ2.0//Black Jack Game 2.0//images//PNG";
    ...
    public void startGame()
    {
        if(can_start)
        {
            ...
            //To test for black jack in user cards(first win situation,the following code is not needed in the final implementation)
                // pickedCard="AD.png";
                // user_cards[user_card_count]=(new ImageIcon(path+"AD.png").getImage()).getScaledInstance(100, 152, java.awt.Image.SCALE_SMOOTH);
                // user_card_count++;
                // user_card_value+=get_card_value();
                // pickedCard="10D.png";
                // user_cards[user_card_count]=(new ImageIcon(path+"10D.png").getImage()).getScaledInstance(100, 152, java.awt.Image.SCALE_SMOOTH);
                // user_card_count++;
                // user_card_value+=get_card_value();
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if(running)
        {
            ...
            if(can_stand==false)
            {
                g.drawString("Dealer Card Value: "+dealer_card_value,350,30);
                GameOver(g2D);
            }
            //display user cards
            ...
            g.drawString("Your Cards Value: "+user_card_value,350,280);
    ...
    public void GameOver(Graphics g)
    {
        ...
        else if(user_card_value==21 && user_card_count==2)
        {
            ...
            g.setFont(new Font("Calibri",Font.BOLD,20));
            FontMetrics ScoreMetrics=getFontMetrics(g.getFont());
            g.drawString("You Won!!",(SCREEN_WIDTH-ScoreMetrics.stringWidth("You Win!!"))/2,SCREEN_HEIGHT/2+35);
        ...
        }else if(dealer_card_value <= 21 && dealer_card_value > user_card_value){
            g.drawString("Game Over",(SCREEN_WIDTH-GOmetrics.stringWidth("Game Over"))/2,SCREEN_HEIGHT/2+15);
            //For printing the text in the center of the panel
            g.setFont(new Font("Calibri",Font.BOLD,20));
            FontMetrics ScoreMetrics=getFontMetrics(g.getFont());
            ...
            //For printing the text in the center of the panel
            g.setFont(new Font("Calibri",Font.BOLD,20));
            g.drawString("Tie!",(SCREEN_WIDTH-GOmetrics.stringWidth("Tie!"))/2,SCREEN_HEIGHT/2+15);
        }
        for(int i=0;i<user_card_count;i++)
        {

...
public class GamePanel extends JPanel implements ActionListener{
    ...
    Image user_bet_chips[]=new Image[20];
    ...
    static int bet_chipX=730;
    static int bet_chipY=50;
    static int CHIP_SHIFT_VALUE=10;
    static int SHIFT_VALUE=50;
    int user_chip_count=0;
    boolean running=true,can_hit=false,can_start=true,can_stand=false,can_compare=false,won_the_round=false;
    boolean can_bet=true;
    ...
    public GamePanel()
    {
        ...
        bet200=new JButton("Bet $200");
        startgame.setBounds(565,350,100,30);
        stand.setBounds(565,400,100,30);
        hit.setBounds(565,450,100,30);
        bet50.setBounds(715,350,100,30);
        bet100.setBounds(715,400,100,30);
        bet200.setBounds(715,450,100,30);
        this.add(startgame);
        this.add(hit);
        this.add(stand);
        this.add(bet50);
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if(running)
        {
            ...
            g2D.fillRoundRect(700,50,130,170,30,30);
            g.setColor(Color.red);
            g.setFont(new Font("Impact",Font.PLAIN,15));
            g.drawString("Win Count: "+BlackJackMain.wincount, 730, 45);
            g.drawString("Bet Amount: $ "+bet_amount,700,270);
            g.drawString("User Money: $ "+BlackJackMain.UserMoney,700,295);
            //only after the user clicks stand button
            if(can_stand==false && user_card_count>=1)
            {
                g.drawString("Dealer Card Value: "+dealer_card_value,550,45);
                GameOver(g2D);
            }
            //display user cards
            ...
            g.setFont(new Font("Impact",Font.PLAIN,15));
            g.drawString("Your Cards Value: "+user_card_value,550,295);
            ...
            dealer_hit_cardX=40;
            user_hit_cardX=40;
            for(int i=0;i<user_chip_count;i++)
            {
                g.drawImage(user_bet_chips[i],bet_chipX,bet_chipY,this);
                bet_chipY+=CHIP_SHIFT_VALUE;
            }
            bet_chipY=50;
            if(user_chip_count>=10)
            {
                g.drawString("MAX BET", 740, 240);
            }
            if(bet_amount==0 && user_card_count==0)
            {
                g.setFont(new Font("Calibri",Font.BOLD,40));
                FontMetrics ScoreMetrics=getFontMetrics(g.getFont());
                g.drawString("First Bet",(SCREEN_WIDTH-ScoreMetrics.stringWidth("First Bet"))/2,SCREEN_HEIGHT/2+15);
            }
            if(BlackJackMain.UserMoney<0)
            {
                g.setFont(new Font("Calibri",Font.BOLD,12));
                g.drawString("You owe Casino: ",700,320);
                g.drawString("$"+(-BlackJackMain.UserMoney),730,340);
            }
        }
        else
        {
    ...
    public void place_chips(int n)
    {
        for(int i=0;i<n;i++)
        {
            user_bet_chips[user_chip_count]=(new ImageIcon(path+"chips.png").getImage()).getScaledInstance(70, 51, java.awt.Image.SCALE_SMOOTH);
            user_chip_count++;
        }
    }
    ...
    public void GameOver(Graphics g)
    {
        ...
        g.fillRoundRect(700,50,130,170,30,30);
        g.setColor(Color.red);
        ...
        else if(user_card_value==21 && user_card_count==2)
        {
            ...
            g.setFont(new Font("Impact",Font.PLAIN,15));
            g.drawString("+ "+2*(bet_amount),780,315);
            won_the_round=true;
        }
        ...
        else if(!can_stand)
        {
            ...
            else if(dealer_card_value == user_card_value)
            {
                ...
                g.setFont(new Font("Impact",Font.PLAIN,15));
                g.drawString("+ "+(bet_amount),780,315);
                BlackJackMain.UserMoney+=(bet_amount);
            }
        }
    ...
    public void actionPerformed(ActionEvent ae)
    {
        ...
        else if(command=="Start Game")
        {
            ...
            //to restart the game if start game button is clicked again after the round ends 
            //it creates a new object of the gameframe class
            else if(can_stand==false && can_hit==false){
                startgamecount=0;
                if(won_the_round)
                {
                    BlackJackMain.wincount++;
                    BlackJackMain.UserMoney+=2*(bet_amount);
                }
                if(BlackJackMain.UserMoney<=0)
                {
                    can_bet=false;
                }
                BlackJackMain.game.dispose();
                BlackJackMain.game=new GameFrame();
            }
        ...
        else if(command=="Bet $50")
        {
            if(can_bet)
            {
                ...
                else if(BlackJackMain.UserMoney>=0)
                {
                    BlackJackMain.UserMoney-=50;
                    bet_amount+=50;
                    this.place_chips(1);
                    repaint();
                }
